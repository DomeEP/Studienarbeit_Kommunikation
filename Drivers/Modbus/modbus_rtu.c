/**
  ...
  * - T3.5 character timeout via TIM6 (Hardware Timer).
 ...
 static void Modbus_Restart_RX(Modbus_Handle_t *hmodbus) {
    // 1. Laufenden Empfang sicher abbrechen
    HAL_UART_AbortReceive(hmodbus->huart);
    
    // 2. Hardware Error Flags und Idle Flag hart löschen
    __HAL_UART_CLEAR_FLAG(hmodbus->huart, UART_CLEAR_OREF | UART_CLEAR_NEF | UART_CLEAR_FEF | UART_CLEAR_PEF | UART_CLEAR_IDLEF);
    
    // 3. RX FIFO verwerfen und RDR Register manuell leeren falls nötig
    ...
    // Einzelbyte-Empfang starten. TIM6 übernimmt die Paketerkennung (T3.5).
 ...
 static void Modbus_Send(Modbus_Handle_t *hmodbus, uint16_t length) {
     // Timer stoppen während wir senden
 ...
 HAL_StatusTypeDef Modbus_Master_Request(Modbus_Handle_t *hmodbus, uint8_t slave_id, uint8_t func_code, uint16_t reg_addr, uint16_t reg_val) {
     ...
     // Broadcast (ID 0) braucht keine Antwort!
     if (slave_id == 0) {
         hmodbus->state = MB_STATE_IDLE;
     } else {
 ...
 static void Modbus_SendException(Modbus_Handle_t *hmodbus, uint8_t func, uint8_t exc) {
     ...
     HAL_Delay(20); // RS485 Turnaround Delay
 ...
 void Modbus_IRQHandler_RxCplt(Modbus_Handle_t *hmodbus) {
     ...
     // Timer neu starten für T3.5 Erkennung
     hmodbus->htim->Instance->CNT = 0;
     HAL_TIM_Base_Start_IT(hmodbus->htim);
     
     // Nächstes Byte erwarten
     HAL_UART_Receive_IT(hmodbus->huart, &hmodbus->rx_buffer[hmodbus->rx_index], 1);
 }
 
 void Modbus_IRQHandler_Timeout(Modbus_Handle_t *hmodbus) {
     // Timer stoppen -> Frame ist komplett!
     HAL_TIM_Base_Stop_IT(hmodbus->htim);
