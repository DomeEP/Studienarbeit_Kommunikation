 ...
 * @date 2026-02-21
 *
 * Supports FC03, FC06, FC16. Frame detection via hardware timer (T3.5).
 * RS485 direction control via USART hardware DE pin.
 */
...
/* Modbus CRC16 table (polynomial 0xA001) */
...
static void Modbus_Restart_RX(Modbus_Handle_t *hmodbus) {
    ...
    while (hmodbus->huart->Instance->ISR & USART_ISR_RXNE_RXFNE) {
        (void)hmodbus->huart->Instance->RDR;
    }
...
/**
 * Auto-calibrate timer ARR to 1.75ms (T3.5 at 115200 baud).
 * Reads the actual APB clock and prescaler from hardware registers.
 */
static void Modbus_AutoConfigTimer(Modbus_Handle_t *hmodbus) {
    ...
    /* Determine APB bus (APB1 vs APB2) based on peripheral base address */
    if ((uint32_t)hmodbus->htim->Instance >= APB2PERIPH_BASE) {
        apb_freq = HAL_RCC_GetPCLK2Freq();
        tim_clock = ((RCC->CFGR & RCC_CFGR_PPRE2) == RCC_CFGR_PPRE2_DIV1) ? apb_freq : 2 * apb_freq;
    } else {
        apb_freq = HAL_RCC_GetPCLK1Freq();
        tim_clock = ((RCC->CFGR & RCC_CFGR_PPRE1) == RCC_CFGR_PPRE1_DIV1) ? apb_freq : 2 * apb_freq;
    }
...
HAL_StatusTypeDef Modbus_Master_Request(Modbus_Handle_t *hmodbus, uint8_t slave_id, uint8_t func_code, uint16_t reg_addr, uint16_t reg_val) {
    ...
    if (slave_id == 0) {
        hmodbus->state = MB_STATE_IDLE;  /* Broadcast: no response expected */
    } else {
...
Modbus_Error_t Modbus_Master_Process(Modbus_Handle_t *hmodbus) {
    ...
    if (hmodbus->frame_complete) {
        ...
        if (hmodbus->pending_func_code == MB_FC_READ_HOLDING_REGISTERS) {
            ...
            if (byte_count >= 2) register_map[0] = (hmodbus->rx_buffer[3] << 8) | hmodbus->rx_buffer[4];
            if (byte_count >= 4) register_map[1] = (hmodbus->rx_buffer[5] << 8) | hmodbus->rx_buffer[6];
            if (byte_count >= 6) register_map[2] = (hmodbus->rx_buffer[7] << 8) | hmodbus->rx_buffer[8];
        }
        ...
        switch (func_code) {
            case MB_FC_READ_HOLDING_REGISTERS:
                if (start_addr + count_val > map_size) {
                    ...
                    hmodbus->tx_buffer[0] = hmodbus->slave_id;
                    hmodbus->tx_buffer[1] = func_code;
                    hmodbus->tx_buffer[2] = count_val * 2;
            ...
            case MB_FC_WRITE_SINGLE_REGISTER:
                if (start_addr >= map_size) {
                    ...
                    if (hmodbus->write_reg_cb) hmodbus->write_reg_cb(start_addr, count_val);
                    for (int i = 0; i < 8; i++) hmodbus->tx_buffer[i] = hmodbus->rx_buffer[i];
                    HAL_Delay(20);
...
void Modbus_IRQHandler_RxCplt(Modbus_Handle_t *hmodbus) {
    if (hmodbus->rx_index < MODBUS_MAX_ADU_SIZE - 1) {
        hmodbus->rx_index++;
    }

    /* Reset T3.5 timer on each received byte */
    hmodbus->htim->Instance->CNT = 0;
